...
# data_dir = f"/home/gil/Map/{mode}/" # should end with / 
data_dir = f"/home/gil/sparoLab/Research/Map_merging/dataset/inha_full2/{mode}/" # should end with / 
...
num_points_in_a_scan = 300000 # for reservation (save faster) // e.g., use 150000 for 128 ray lidars, 100000 for 64 ray lidars, 30000 for 16 ray lidars, if error occured, use the larger value.

is_live_vis = True # recommend to use false 
is_o3d_vis = True
intensity_color_max = 200

is_near_removal = True
thres_near_removal = 10 # meter (to remove platform-myself structure ghost points)
...
poses = []
# f = open(data_dir+f"{mode}_central_aft_intersession_loops.txt", 'r')
f = open(data_dir+f"{mode}_central_aft_intersession_loops.txt", 'r')
